...
internal_neurons_number = 10  # number of neurons and internal nodes
min_MAE_distance = 0.0005  # early stopping when MAE(n)-MAE(n-1) < min_MAE_distance
learning_rate_value = 0.1  # value for lr parameter
momentum_value = 0.8  # value for momentum parameter
...
for (train_index, test_index) in kf.split(X):
    ...
    model.add(Dense(internal_neurons_number, activation="relu", activity_regularizer=l1(regularizer_value), input_dim=len(X)))  # internal level
    model.add(Dense(len(Y[0]), activation="sigmoid",  input_dim=internal_neurons_number))  # output level
